...
def train_model(target, num, n):
    ...
    model.fit(x_train, y_train,
              ...
              verbose=2,
              validation_data=(x_test, y_test))
    model.save(f"./result/model_test_{mode}/{target}/{num}_weights.h5")
...
def load(target):
    model_list = []
    for i in range(3):
        m = load_model(f"./result/model_test_{mode}/{target}/{i}_weights.h5")
        model_list.append(m)
    return model_list
...
def save_data(data_path, temp_b, temp_n, n):
    ...
    for item_a, item_n in zip(temp_b, temp_n):
        ...
        x_a_n.append(temp)
        y_a_n.append(k)
    save_path = f"./result/data_test_{mode}/" + data_path + f"/{n}"
