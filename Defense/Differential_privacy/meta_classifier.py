...
def attack_train():
    ...
    attack_model.fit(x_train, y_train, epochs=30, batch_size=128, validation_data=(x_test, y_test))
...
def attack_test():
    # test_model = load_model("./attack_model_896.h5")
    # path = f"./data_test_{mode}/list"
    test_model = load_model("./attack_model.h5")
    path = f"./result/data_test_0.05/list"
...
def load_test_data_10(data_path, num):
    print("load data...")
    x_train_attack = []
    y_train_attack = []
    x_test_attack = []
    y_test_attack = []
    for j in range(100):
        read_path = data_path + f"/{num}/{j}.jpg"
        x_temp = img.imread(read_path)
        # print(x_temp)
        if j < 100:
            x_train_attack.append(x_temp)
            y_train_attack.append(num)
        else:
            x_test_attack.append(x_temp)
            y_test_attack.append(num)
    x_train_attack = np.array(x_train_attack)
    y_train_attack = np.array(y_train_attack)
    x_test_attack = np.array(x_test_attack)
    y_test_attack = np.array(y_test_attack)
...
def attack_test_10():
    test_model = load_model("./attack_model.h5")
    path = f"./result/data_test_0.25/list"
    for i in range(10):
        x, y, x_, y_ = load_test_data_10(path, i)
        x_test = x.astype('float32')
        x_test /= 255
        y_test = np_utils.to_categorical(y, 10)
        x_test = x_test.reshape(x_test.shape[0], 10, 320, 1)
