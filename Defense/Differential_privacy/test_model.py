...
def initial_model(train_x, train_y, test_x, test_y):
    ...
    model.save(f"./model_weights_aggregation_0.05.h5")
...
def train(train_x, train_y, test_x, test_y):
    model = create_model()
    m = load_model(f"model_weights_aggregation_{epsilon}.h5")
    w = m.get_weights()
    model.set_weights(w)
    train_x = train_x.astype('float32')
    test_x = test_x.astype('float32')
...
def add_noise(x_list):
    ...
    for x in x_list:
        ...
        noise = np.random.normal(0, sigma / math.sqrt(5), x.shape)
        new_x = x + noise
...
def get_input(train_x, train_y, test_x, test_y, w):
    print("get input...")
    train_x = train_x.astype('float32')
    test_x = test_x.astype('float32')
    train_x /= 255
...
def save_data(data_path, temp_b, temp_n):
    ...
    for item_a, item_n in zip(temp_b, temp_n):
        ...
        x_a_n.append(temp)
        y_a_n.append(k)
    save_path = f"./result/data_test_{epsilon}/" + data_path + f"/{k}"
...
if __name__ == '__main__':
    ...
    for k in range(10):
        ...
        model_a.save(f"./result/model_test_{epsilon}/{k}/model_weights_aggregation_{epsilon}.h5")
        x_train_get, y_train_get = distribute_dataset_get(x_train, y_train, 1000)
        g_a, g_a_list = get_input(x_train_get, y_train_get, x_test, y_test, model_a.get_weights())
        model_next = train_next(x_train_a, y_train_a, x_test, y_test, model_a)
        g_n, g_n_list = get_input(x_train_get, y_train_get, x_test, y_test, model_next)
        path_1 = "list"
        path_2 = "num"
