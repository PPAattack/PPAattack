...
if __name__ == '__main__':
    initial_model()
    user = load_dataset.get_client_dataset(200, 3000)
    for i in range(len(user)):
        new_weights = train_model(user[i].x_train, user[i].y_train, user[i].x_test, user[i].y_test)
        g_be, g_list_be = get_input(new_weights)
        fp = open(f"sensitivity_{i}.txt", "a+")
        print(f"model_sensitivity:\n{g_be}", file=fp)
        fp.close()
